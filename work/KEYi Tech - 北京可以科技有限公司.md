...
将电池样品送给电量计芯片厂商，厂商对电池数据进行建模，得到一组数据，更新到设备树文件中。

根据需求修改电量查询逻辑，提供平稳的电量降低曲线。
...
将官方提供的驱动文件，移植适配到当前的内核版本，并根据需求修改功能。

## 电机控制MCU 的 SPI 驱动开发

根据定义好的协议通信，使用spidev驱动通信，逻辑直接放到app中。

## 4. 驱动电源管理，低功耗休眠以及测试工具的开发。

为 linux 上的外设驱动添加 PM 电源管理接口，通过 power/state 发起调用流程。添加PM接口的驱动包括但不限于：Display，电机，MCU等。

编写的压力测试工具用于测试反复进入退出休眠模式时，系统是否还可以正常运行，APP没有异常退出。
