当你访问总线上不存在的一片内存地址空间时，会触发 HardFault，我们可以利用这个机制，将一个 spi psram 模拟的映射到总线上，只需要在 Hardfault 的处理程序中，保存现场，以及触发 hardfault 的那一条指令，比如向某一个地址写了某个值，读取某个地址的值，我们解析这些指令，保存进入hardfault时的现场，切换到一个外部的c函数中，访问spi psram来进行读取或写入，返回，然后恢复现场。 这样一来，你编写的程序就以为0x2000000 ~ 0x20000040（举个例子）这片空间是映射到总线上的，是可以访问的。只不过由于频繁上下文切换，导致读写性能比较低。
